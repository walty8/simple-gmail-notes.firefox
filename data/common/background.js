...
function launchAuthorizer(sender, callback) {
  throw "launchAuthorizer not implemented";
}

function removeCachedToken(tokenValue){
  throw "removeCachedToken not implemented";
}
 ...
 */
function debugLog() //need some further work
{
...
function loginGoogleDrive(sender, messageId){
  ...
  launchAuthorizer(sender, function(redirect_url) {
      if(!redirect_url){
...
function logoutGoogleDrive(sender){
  ...
  if(tokenValue){
    ...
    removeCachedAuthToken(tokenValue);
    sendAjax({
...
function searchNote(sender, messageId){
  executeIfValidToken(sender, function(data){
...
function setupListeners(sender, request){
  debugLog("Message from a content script:" + sender.tab.url);
