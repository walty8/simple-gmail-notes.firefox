...
(function()
{
    ...
    this._openDialog = function(location) {
        ...
        var isAuthWindowAlive = true;
        try { //could not find other ways to detect a dead object
          var dummy = JSON.stringify(this.authWindow);
        }
        catch(e){
          if(e.message.indexOf(' dead') != -1){
            isAuthWindowAlive = false;
          }
        }

        if (isAuthWindowAlive && this.authWindow && !this.authWindow.closed) {
            ...
            this.authWindow = openDialog({url: location,
                                          name: "oauth_authorization_dialog",
                                          // ideally we would use 'modal', but then we can't get a window ref...
                                          features: "location=yes,centerscreen,dialog=no,width=800,height=540,resizable=yes"});
        }
    ...
    this.openDialog = function(loginUrl, requestData, svc, afterAuthCallback) {
        ...
        console.log("oauth listner to be opened");
